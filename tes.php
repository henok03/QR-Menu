...
<head>
    ...
    <style>
        ...
        #menuSection { background: #0f1923; }
        #categoryContainer { background: #0f1923 !important; }
        ...
        .cb-hot     { background: #d9420b; }
        .cb-cold    { background: #1a8a6e; }
        .cb-burger  { background: #c47d0e; }
        .cb-pizza   { background: #b03030; }
        .cb-dessert { background: #6b3dbb; }
        .cb-default { background: #2d6a4f; }

        .card-body {
            padding: 14px 14px 0 14px; flex: 1;
        ...
        .card-icon-circle i { font-size: 1rem; }
        .ci-hot     { border-color: #d9420b; color: #d9420b; }
        .ci-cold    { border-color: #1a8a6e; color: #1a8a6e; }
        .ci-burger  { border-color: #c47d0e; color: #c47d0e; }
        .ci-pizza   { border-color: #b03030; color: #b03030; }
        .ci-dessert { border-color: #6b3dbb; color: #6b3dbb; }
        .ci-default { border-color: #09b111; color: #09b111; }
        ...
        .cl-hot     { color: #d9420b; }
        .cl-cold    { color: #1a8a6e; }
        .cl-burger  { color: #c47d0e; }
        .cl-pizza   { color: #b03030; }
        .cl-dessert { color: #6b3dbb; }
        .cl-default { color: #09b111; }
        .card-desc {
            font-size: 0.75rem; color: #8a97a8; line-height: 1.5;
            display: -webkit-box; -webkit-line-clamp: 2;
            -webkit-box-orient: vertical; overflow: hidden;
        }
        .card-price-bar {
            margin-top: 12px; background: #09b111;
        ...
        .card-fav-btn:hover { transform: scale(1.35); color: #ffcdd2; }
        .card-fav-btn.liked { color: #ff6b6b; }
    </style>
</head>
...
<nav class="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md shadow-sm px-4 py-3">
    ...
    <section class="hero-section">
        ...
        <div class="hero-left">
            ...
            <div class="hero-btns">
                ...
                <button onclick="scrollToMenu()" class="hero-btn-secondary">
            ...
            <div class="hero-cat-pills">
                <div class="hero-cat-pill" onclick="setCategory('burgers');scrollToMenu();">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 11h18"/><path d="M3 7h18c0-2.2-4-4-9-4S3 4.8 3 7z"/><path d="M3 15h18c0 2.2-4 4-9 4s-9-1.8-9-4z"/></svg>
                    <span>Burgers</span>
                </div>
                <div class="hero-cat-pill" onclick="setCategory('pizza');scrollToMenu();">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2C6.5 2 2 6 2 12h20c0-6-4.5-10-10-10z"/><path d="M2 12c0 4.4 3.6 8 8 8s8-3.6 8-8"/></svg>
                    <span>Pizzas</span>
                </div>
                <div class="hero-cat-pill" onclick="setCategory('desserts');scrollToMenu();">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10H3l3 10h12l3-10z"/><path d="M12 4V2"/></svg>
                    <span>Desserts</span>
                </div>
                <div class="hero-cat-pill" onclick="setCategory('all');scrollToMenu();">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 8h1a4 4 0 110 8h-1"/><path d="M3 8h14v9a4 4 0 01-4 4H7a4 4 0 01-4-4V8z"/><path d="M6 2v4"/><path d="M10 2v4"/><path d="M14 2v4"/></svg>
                    <span>Drinks</span>
                </div>
                <div class="hero-cat-pill" onclick="setCategory('hot_drinks');scrollToMenu();">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8h1a4 4 0 010 8h-1"/><path d="M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4V8z"/><path d="M6 1v3"/><path d="M10 1v3"/><path d="M14 1v3"/></svg>
                    <span>Hot Drinks</span>
                </div>
                <div class="hero-cat-pill" onclick="setCategory('cold_drinks');scrollToMenu();">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M7 21h10"/><path d="M12 21V11"/><path d="M5 11h14l-2-7H7l-2 7z"/></svg>
                ...
                <div class="hero-phone-screen">
                    ...
                    <div class="hero-phone-header">
                        <div class="hero-phone-logo">
                            ...
                            <span class="hero-phone-logo-text">PandaKitchen</span>
                        </div>
    ...
    <!-- ================= MENU SECTION ================= -->
    <main id="menuSection" class="flex-grow w-full px-4 py-8" style="background:#0f1923; max-width:100%;">
        <div class="max-w-4xl mx-auto">
        <!-- Categories -->
        <div class="flex overflow-x-auto gap-3 mb-8 pb-2 no-scrollbar sticky top-[70px] z-40 pt-2" id="categoryContainer" style="background:#0f1923;">
            <!-- Categories injected by JS -->
        </div>

        <!-- PHP Food Items Loop -->
        <div id="foodGrid" class="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <?php if (!empty($items)): ?>
                <?php foreach ($items as $index => $item): ?>
                    <?php
                        ...
                        $catRaw = strtolower(trim($item['category']));

                        // Determine colour/icon/badge key
                        if (strpos($catRaw, 'hot') !== false) {
                            $key = 'hot';
                            $badgeLabel = 'HOT';
                            $badgeIcon  = 'fa-fire';
                            $itemIcon   = 'fa-mug-hot';
                        } elseif (strpos($catRaw, 'cold') !== false) {
                            $key = 'cold';
                            $badgeLabel = 'COLD';
                            $badgeIcon  = 'fa-snowflake';
                            $itemIcon   = 'fa-glass-water';
                        } elseif (strpos($catRaw, 'burger') !== false || strpos($catRaw, 'burger') !== false) {
                            $key = 'burger';
                            $badgeLabel = 'BURGER';
                            $badgeIcon  = 'fa-burger';
                            $itemIcon   = 'fa-burger';
                        } elseif (strpos($catRaw, 'pizza') !== false) {
                            $key = 'pizza';
                            $badgeLabel = 'PIZZA';
                            $badgeIcon  = 'fa-pizza-slice';
                            $itemIcon   = 'fa-pizza-slice';
                        } elseif (strpos($catRaw, 'dessert') !== false) {
                            $key = 'dessert';
                            $badgeLabel = 'DESSERT';
                            ...
                            $key = 'default';
                            $badgeLabel = strtoupper($item['category']);
                    ...
                    <div class="food-card"
                        ...
                        <div class="card-img-wrap">
                            <img src="<?php echo htmlspecialchars($item['image_path']); ?>"
                                 ...
                                 loading="lazy">
                            <span class="card-badge cb-<?php echo $key; ?>">
                        ...
                        <div class="card-body">
                            <div class="card-icon-row">
                                <div class="card-icon-circle ci-<?php echo $key; ?>">
                                    <i class="fas <?php echo $itemIcon; ?>"></i>
                                </div>
                                <div class="card-text">
                                    <div class="card-name"><?php echo htmlspecialchars($item['name']); ?></div>
                                    <div class="card-cat-label cl-<?php echo $key; ?>"><?php echo htmlspecialchars($item['category']); ?></div>
                        ...
                        <div class="card-price-bar">
                            <span class="card-price-text"><?php echo number_format((float)$item['price'], 2); ?> ETB</span>
                            <button class="card-fav-btn" onclick="this.classList.toggle('liked')" title="Add to favourites">
            ...
            <?php else: ?>
                <div class="col-span-full text-center py-10">
                    <i class="fas fa-utensils text-5xl text-gray-600 mb-4"></i>
        ...
        <div id="emptyState" class="hidden text-center py-10">
            <i class="fas fa-cookie-bite text-6xl text-gray-600 mb-4 animate-bounce"></i>
            <p class="text-xl text-gray-400" data-i18n="noItems">No items found.</p>
            <button onclick="clearFilters()" class="mt-4 text-green-400 hover:underline">Clear filters</button>
        </div>
        </div><!-- end max-w-4xl -->
    </main>

    <!-- ================= FOOTER ================= -->
    <footer style="background:#0a1220; border-top:1px solid rgba(255,255,255,0.07);">
